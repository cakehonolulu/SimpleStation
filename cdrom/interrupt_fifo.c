...
uint8_t m_cdrom_interrupt_fifo_pop(m_simplestation_state *m_simplestation)
{
    ...
    else
    {
        ...
        {
            m_simplestation->m_cdrom->m_interrupt_fifo_index--;
            m_interrupt = m_simplestation->m_cdrom->m_interrupt_fifo[m_simplestation->m_cdrom->m_interrupt_fifo_index];
        }
        else
        {
            printf(RED "[CDROM] interrupt_fifo_pop: FIFO Index Underflow, aborting...!\n" NORMAL);
            m_simplestation_exit(m_simplestation, 1);
        }
    }
