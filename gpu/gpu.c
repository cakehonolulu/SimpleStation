...
gp0_mode m_gp0_read_mode = command;
uint32_t m_vramReadBufferSize = 0;
uint32_t m_vramReadBufferIndex = 0;

void readDataMem(uint32_t* destination, int size, m_simplestation_state *m_simplestation) {
...
uint32_t m_gpu_get_read(m_simplestation_state *m_simplestation)
{
    ...
    if (m_gp0_read_mode == vram_to_cpu)
    {
        readDataMem(&m_read, 1, m_simplestation);
        //m_read = 0x69;
    }
